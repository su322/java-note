...
public class JavaFundamentals {
	public static void main(String[] args) {
		...
		*/
		...
		//单列集合的祖宗接口Collection 接口List的实现类ArrarList LinkedList Vector （淘汰）接口Set的实现类HashSet LinkedHashSet TreeSet
		...
		//ArrayList底层
		//1、利用空参创建的集合，在底层创建默认长度为0的数组 2、添加第一个元素时，底层会创建一个新的长度为10的数组 3、存满时，会扩容1.5倍 （老的10加上10的一半）4、如果一次添加多个元素，1.5倍还放不下，则新创建数组的长度以实际为准 看视频很清晰
		//LinkedList底层 双向链表，查询慢，增删快，但是如果操作的是首尾元素，速度也是极快的。
		//特有方法 addFirst addLast getFirst getLast removeFirst removeLast 用得少
		//LinkedList的源码，看的跟当时学数据结构一个感觉
		//Iterator底层实际上就是创建了一个内部类的对象，这个内部类就表示是ArrayList的迭代器，还是看视频清晰
		//如何避免并发修改异常，在使用迭代器或者是增强for遍历集合的过程中，不要使用集合的方法去添加或者修改元素即可
		
		//泛型：是jdk5中引入的特性，可以在编译阶段约束操作的数据类型，并进行检查，<>，只支持引用数据类型
		//如果我们没有给集合指定类型，默认认为所有的数据类型都是Object类型，此时可以往集合中添加任意的数据类型，带来一个坏处：我们在获取数据的时候，无法使用它的特有方法。此时推出了泛型，可以在添加数据的时候就把类型统一，而且我们在获取数据的时候，也省得强转了。
		//Java中的泛型是伪泛型，java文件编译成class文件之后，泛型标记会消失，这叫泛型的擦除，底层其实只是在创建集合的时候限制了类型，实际上还是Object类型，再转换成限制的类型。
		...
		//2、public class MyArrayList<E> implements List<E>{}
		
		//泛型不具备继承性，但数据具备继承性，比如方法的参数写<Ye>，可以传递<Ye>，不能传递<Fu><Zi>，但是add()可以传new <Ye><Fu><Zi>
		//利用泛型方法有一个弊端，他可以接受任意的数据类型
		//泛型的通配符：<?>和E差不多，只不过前面修饰符后不用再写，<? extends E>可以传递E或E的所有子类，<? super E>可以传递E或E的所有父类
		//如果类型不确定，但是能知道以后只能传递某个继承体系中的，就可以用泛型的通配符
		
		//度：每一个节点的子节点数量
		//二叉查找（排序、搜索）树：每一个节点最多有两个子节点，任意节点左子树上的值都小于当前节点，任意节点右子树上的值都大于当前节点
		//先（前）序遍历：中左右 中序遍历：左中右 后序遍历：左右中 （取名方式是来自于根节点的位置）层序遍历：一层一层的
		//平衡二叉树：任意节点左右子树高度差不超过1
		//旋转机制：左旋 右旋 触发时机：当添加一个节点之后，该树不再是一颗平衡二叉树
		//确定支点：从添加的节点开始，不断地往父节点找不平衡的节点
		//左旋：把不平衡的点作为支点，把支点左旋降级，变成左子结点，晋升原来的右子结点
		//复杂点，如果找到了根节点作为支点，将根节点的右侧往左拉，原先的右子节点变成新的父节点，并把多余的左子结点出让，给已经降级的根结点右子节点
		//需要旋转的四种情况：视频讲的真好
		//左左（根节点左子树的左边）：一次右旋
		//左右：先局部左旋，再整体右旋
		//右右：刚好相反
		...
		//红黑树：一种自平衡的二叉查找树，1972年出现，当时被称为平衡二叉B树，后来1978年被修改为现在的红黑树。它是一种特殊的二叉查找树，每一个节点上都有存储位表示节点的颜色，每一个节点可以是红或者黑，红黑树不是高度平衡的，它的平衡是通过红黑规则进行实现的。
		//红黑规则：1、每一个节点是红色的，或者是黑色的 2、根节点必须是黑色 3、叶节点是黑色的 4、两个红色节点不能相连 5、任意节点到所有后代叶节点的简单路径上，黑色节点数量相同
		//添加节点默认颜色是红色的效率高，调整次数更少
		//添加节点的规则挺复杂的，但是走流程也简单，看视频
		//红黑树增删改查性能都很好
		
		//Set集合的实现类 HashSet：无序、不重复、无索引 LinkedHashSet：有序、不重复、无索引 TreeSet：可排序、不重复、无索引
		//Set接口中的方法基本上与Collection的API一致
		//HashSet集合底层采取哈希表 jdk8之前：数组+链表 jdk8开始：数组+链表+红黑树
		//哈希值：根据hashCode方法算出来的int类型的整数，默认使用地址值进行计算，如果集合中存储的是自定义对象，必须重写equals方法和hashCode方法，利用对象内部的属性值计算哈希值，如果没有重写，不同对象计算出的不同，如果重写，不同对象只要属性值相同，计算出的哈希值一样。
		//哈希冲突：小部分情况下，不同的属性值或者不同的地址值计算出来的哈希值也有可能一样
		
		//HashSet底层原理：创建一个默认长度为16，默认加载因子为0.75的数组，数组名table。根据元素的哈希值跟数组的长度计算出应存入的位置。判断当前位置是否为null，如果是null直接存入。如果位置不为null，表示有元素，则调用equals方法比较属性值。如果一样就不存，如果不一样就存入数组，形成链表，这个过程在jdk8以前是新元素存入数组，老元素挂在新元素下面，jdk8以后是新元素直接挂在老元素下面。
		//默认加载因子是扩容的时机，也就是存了16*0.75=12个的时候，就扩容成两倍，当链表长度大于8并且数组长度大于等于64时，链表会自动转成红黑树
		
		//LinkedHashSet底层依然是哈希表，只是每个元素又额外的多了一个双向链表的机制记录存储的顺序，取的时候就从头结点开始就保证了存取元素顺序一致
		...
		//异常有运行时异常和编译时异常 编译时异常要写，运行时异常可以省略
		//try catch可以让程序继续往下执行，不会停止
		//如果try种可能会遇到多个问题，写多个catch与之对应，这些异常中如果存在父子关系的话，那么父类一定要写在下面 jdk7以后能写|符号来把相同处理的异常写到一起
		//try中遇到的问题没有被捕获，相当于try catch白写了
		//try中遇到了问题，下面的代码就不会执行了，直接跳转到catch中
		//e.printStackTrace() 底层利用System.err.println 红色字体输出 仅仅打印信息，不会停止程序运行。
		//throws写在方法定义处 ，告诉调用者，可能会有哪些异常 throw写在方法内，结束方法，手动抛出异常对象，交给调用者，下面的代码不再执行了
		//自定义异常步骤：定义异常类、写继承关系、空参构造、带参构造 意义：让控制台的报错信息更加的见名知意 继承运行时：RuntimeException 表示由于参数错误而导致的问题 继承编译时：Exception 提醒程序员检查本地信息
		...
		//锁的嵌套容易导致死锁
		//java虚拟机中线程有六种状态 NEW新建 RUNNABLE就绪 BLOCKED阻塞 WAITING等待 TIMED_WATING及时等待 TERMINATED退出 这些状态是虚拟机状态，不反应任何操作系统线程状态
		//自定义线程池 构造方法有七个参数
		//创建一个空的池子，有任务提交时，线程池会创建线程去执行任务，执行完毕归还线程。当核心线程满时，在提交任务就会排队。当核心线程满，队伍满时，会创建临时线程。当核心线程满，队伍满，临时线程满时，会触发任务拒绝策略。
		//线程池多大合适？CPU密集型运算：最大并行数+1 I/O密集型运算：...
		
		//反射允许对成员变量，成员方法和构造方法的信息进行编程访问
		//获取class对象：1、Class.forname("") 最常用 2、类名.class 一般更多是当做参数进行传递 3、对象.getClass() 已经有了这个类的对象才可以使用
		//可以用获取到的字节码文件对象.get... 获取构造方法、参数、成员变量、成员方法等 可以创建对象 newInstance 暴力反射 setAccessible(true)
		//ctrl+p看方法参数
		
		//动态代理：可以无侵入式的给代码增加额外的功能
		//调用者->代理->对象
		//通过接口保证，后面的对象和代理需要实现同一个接口，接口中就是被代理的所有方法 Proxy.newProxyInstance(类加载器, 接口数组, 要干什么) 代码比较复杂，可以看视频
		}
	}
}